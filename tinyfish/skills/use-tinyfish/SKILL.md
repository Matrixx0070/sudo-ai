...
## Getting Unstuck (consent walls & CAPTCHAs)

Live sites throw up blockers that stall agents. Baseline behavior for `agent` and `browser` runs:

- **Dismiss consent/cookie/age walls automatically** — make it the first step of every goal (Google "Before you continue", GDPR banners, region and "open in app" modals). Prefer "Reject all" where offered.
- **On a CAPTCHA or bot check** (reCAPTCHA, hCaptcha, Cloudflare "Checking your browser", press-and-hold): let the real browser clear invisible checks, retry **at most once**, then **hand off to a human** — return a `BLOCKED` status with the URL, or open a viewable `browser session` for the user to solve it, then resume. Never loop on a CAPTCHA; that is the classic stuck state. Logins/2FA/OTP are always user-provided.
- **Every run ends in a defined state** — `DONE` / `BLOCKED` / `FAILED` with a per-step timeout, so nothing hangs.

The full playbook (detection signals, escalation ladder, goal templates) lives in the `browser-tasks` skill at `references/unblocking.md`. Automate only what the user could legitimately do themselves.

...
**User completion polish (docs + E2E):** This wiring enables real-time interaction with the agent directly from the TUI. E2E coverage: TUI sessions (via an expect-based harness) send cross-session, learner, and setup prompts, and the captured logs confirm that tool events flow to the TUI cards and that learner output is visible, with no regressions. See the README TUI section, the TUI spec, and the cross-session guide for user examples. The single-command entry point and setup wizard both land users in this wired TUI. (No source changes here; docs only.)

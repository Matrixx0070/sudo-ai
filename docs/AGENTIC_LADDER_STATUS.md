...
5. AL7.1 remainder — MOSTLY DONE (PR #943 merged; agent tasks scrape-zero-fields + repeated-tool-calls registered; #751 empty-reply = harness rule in AgentBenchRunner; evaluateDraftGate fail-closed; 146/146 eval tests. LEFT OPEN: bench dashboard tab. CORRECTED 2026-07-31: NO nightly bench cron exists — BenchStore/routes are on-demand only, bench.db has 0 rows; "until the cron fires" assumed wiring that was never built. Scheduling a nightly AgentBench = recurring model spend -> Frank gate (invariant 10 budget declaration required); filed to the blocked-on-Frank list)

...
**Authored:** 2026-06-18.
**Driver:** `consciousness:cognitive-stream` was discovered to burn ~$17/hr-equivalent on Opus 4.8 in the running PM2 daemon, out-burning a Max 20x subscription by ~3×. Not safe to ship to normal users.

## The problem

The PM2-running `sudo-ai-v5` daemon's consciousness loop calls the configured model (currently Opus 4.8) roughly every 30 seconds, 24/7, with ~27k-token contexts. From `data/logs/sudo-ai-v5-out-*.log`:

| Metric | Value |
|---|---|
| Calls per hour | ~120 |
| Avg prompt tokens | ~27,000 |
| Estimated cost per call | $0.13–0.14 |
| Continuous burn | **~$17/hr-equivalent ≈ $12,200/month** |
| Out-burns a Max 20x plan ($4–5K/mo effective) by | **~3×** |

A user on a $20/month Claude Max plan would exhaust their monthly quota in **under 2 days**. The "100x autonomous AI for everyone" vision is incompatible with the current consciousness configuration. Nobody can afford it.
...
|---|---|---|---|
| Today | $17/hr | $12,200/mo | Nobody |
| Tier split only (Haiku / Ollama for tiers 1–2, Opus only on tier 3 triggers) | $0.50/hr | $360/mo | 20x Max plan users |
| + Prompt caching | $0.05/hr | $36/mo | Standard Max plan |
| + Sleep cycles + differential gate | **$0.005/hr active, $0 sleeping** | **~$3–5/mo** | $20/mo Claude users — i.e. **everyone** |
...
- Edit `config/sudo-ai.toml` `default_model` to a Haiku / local model, then `pm2 restart sudo-ai-v5 --update-env`. Affects **every** brain call, not just consciousness — use the env override if you want to keep user-facing calls on Opus.

...
- **1.2 F85 verdict = OFF (Frank, 2026-07-19; executed same day — SUDO_PREDICTOR_LOOP=0 live in prod, verified in-process; loop stopped, rows frozen at 1,324; meta.predictor manual tool remains; code retained for a future wire-a-consumer revisit).** mind.db predictions: 1,324 total → 1,320 'incorrect', 0 'correct', 4 pending. Root cause is structural, not model quality: outcomes resolve ONLY via manual meta.predictor record-outcome (never happens) or the expiry sweep marking everything expired as 'incorrect'. Predictions are generated then never consumed or genuinely scored — computed-then-discarded (F122 class). Verdict: either wire a real consumer + outcome recording, or turn SUDO_PREDICTOR_LOOP off; as-is it earns nothing.

...
| `SUDO_ACP_MODEL` | string | unset | Pin a specific model for the ACP agent subprocess (`pnpm acp` / `node dist/core/acp/acp-cli.js`). Default uses the Brain's smart-routing. Slice 2 (gap #26) added tool dispatch + ACP `session/request_permission` round-trip + new `session/update` variants (`tool_call`, `tool_call_update`, `thought`) to the protocol layer; the wire format for tool invocations is `<tool_call id="<stable-id>" name="<tool-name>">JSON-args</tool_call>` blocks in the streamed assistant text, which the backend parses after each stream pass and dispatches via an injected `AcpToolHost`. The permission flow follows ACP: the agent originates `session/request_permission` with the toolCall + standard options (`allow_once`/`allow_always`/`reject_once`) and awaits the client's selection. A real `ToolRegistry` adapter is not yet wired into the standalone ACP entry — that lands in slice 3 alongside `fs/*` and `terminal/*` client methods |

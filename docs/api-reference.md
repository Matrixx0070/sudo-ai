...
| `SUDO_DASHBOARD_DISABLE=1` | Web dashboard (stats/health/alignment/metrics/recent-activity/agents-live). The FleetView endpoint `GET /api/agents/live` (gap #25 slice 1) returns a snapshot of live sub-agents from the multi-agent orchestrator: `{ spawned: AgentSnapshot[], slotsUsed, slotsMax, queueWaiting }`. Each `AgentSnapshot` carries `id`, truncated `task`, `startedAt`, `elapsedMs`, `sinceHeartbeatMs`, and an `idle` flag (true when heartbeat is older than 30s). Prometheus metrics include `sudo_agents_spawned`, `sudo_agents_idle`, `sudo_agents_slots_used`, `sudo_agents_slots_max`, `sudo_agents_queue_waiting`. When the multi-agent orchestrator failed to initialize at boot, the endpoint serves a zero default rather than 500. Slice 2 adds a dedicated TUI (`pnpm tui:fleetview`) — a terminal renderer over the same endpoint, reading `SUDO_DASHBOARD_HOST` (default 127.0.0.1), `SUDO_DASHBOARD_PORT`, and `SUDO_DASHBOARD_TOKEN` (or `GATEWAY_TOKEN` fallback). Optional `SUDO_TUI_POLL_MS` (default 1500, min 250) and `SUDO_TUI_REQUEST_TIMEOUT_MS` (default 4000, min 500) tune polling. Press `q` or Ctrl+C to quit. Slice 3 adds a JSON-RPC 2.0 gateway (`pnpm gateway:fleetview`) — NDJSON-framed over stdio (reuses `src/core/acp/jsonrpc.ts`), proxies the same dashboard endpoints behind dot-named methods: `gateway.version` (handshake → version + method list), `agents.snapshot`, `dashboard.stats`, `dashboard.health`, `dashboard.alignment`, `dashboard.metrics` (Prometheus text parsed into a key→value map), `dashboard.activity` (optional `{limit:1..100}` param, default 50; accepts numeric or stringified-numeric for `jq`/HTTP-to-stdio-bridge compat). Upstream 401 maps to the application-defined error code `GatewayErrorCode.UpstreamUnauthorized` (`-32000`) so clients can distinguish it from the JSON-RPC-reserved `InvalidRequest`; other HTTP errors map to `InternalError`. Optional `SUDO_GATEWAY_REQUEST_TIMEOUT_MS` (default 4000, min 500) caps each upstream call |
| `SUDO_BRAIN_RACE_DISABLE=1` | Brain parallel model race |
| `SUDO_BRAIN_CONSENSUS_DISABLE=1` | 3-model Jaccard consensus |
| `SUDO_BRAIN_OAUTH_STREAM_DISABLE=1` | Streaming of `claude-oauth` completions on the non-streaming `brain.call()` path. On by default: streaming lands response headers in ~1–2s so the fast-fail headers timer (`SUDO_OAUTH_HEADERS_TIMEOUT_MS`, default 45s) can't false-abort long Opus turns. Set to `1` to force the legacy buffered `generateText` path. Non-`claude-oauth` providers always buffer regardless |
| `SUDO_VERIFY_GATE_CACHE=1` | (enabler) TTL-LRU over the verify-gate per-tool confidence lookup so a burst of evaluations of the same destructive tool skips re-opening `audit.db` (+ `calibration.db`). Requires `SUDO_VERIFY_GATE=1`; caching never changes a decision, only lookup frequency. Tunables: `SUDO_VERIFY_GATE_CACHE_TTL_MS` (default 5000, `0` disables), `SUDO_VERIFY_GATE_CACHE_MAX` (default 256 tools) |
| `SUDO_COST_RATE_ALERT=1` | (enabler) Live cost-rate watchdog. Samples $/hour from `api_call_log` on a timer and emits a `cost_rate_alert` hook event when spend crosses an absolute ceiling (`critical`) or deviates sharply above the rolling baseline (`warning`). Observable-only — never blocks spend; fail-open. Tunables: `SUDO_COST_RATE_ALERT_CEILING_USD_PER_HR` (default 10, `0` disables ceiling check), `SUDO_COST_RATE_ALERT_DEVIATION_PCT` (default 150, `0` disables), `SUDO_COST_RATE_ALERT_MIN_USD_PER_HR` (default 0.5 — floor below which deviation alerts are suppressed), `SUDO_COST_RATE_ALERT_INTERVAL_MS` (default 600000), `SUDO_COST_RATE_ALERT_WINDOW_MS` (default 3600000), `SUDO_COST_RATE_ALERT_COOLDOWN_MS` (default 3600000) |
...
| `SUDO_PROMPT_CACHE` | boolean | **on** (set `=0` to disable) | Stable-prefix discipline for provider prompt caches: the volatile Current Date & Time block moves below the system prompt's dynamic boundary, and tool definitions (both the system-prompt tools list and the serialized tool schemas) are sorted by name so the request prefix is byte-identical across calls. Layout-only — no content is added or removed. On Anthropic models it additionally places explicit `cache_control: ephemeral` breakpoints on the last tool definition and on the stable system-prompt prefix (everything above the dynamic boundary); requests to other providers are unchanged. `SUDO_PROMPT_CACHE_BREAKPOINTS_DISABLE=1` keeps the stable prefix but skips the explicit breakpoints |
| `SUDO_WORKFLOWS=1` | boolean | off | Registers `meta.run-workflow`, the deterministic multi-step workflow runner. A workflow is a `.yaml` file of steps under the workspace `workflows/` dir; the engine threads stdout via `stdin: '{{prev}}'`, supports `{{steps.<id>.<field>}}` templating in command/stdin (fields: stdout, stderr, exitCode, status, durationMs), gates steps with `condition` expressions, and pauses on `approval: true` steps returning a resume token. Consecutive steps that share a `parallel_group` label fan out concurrently (bounded by `SUDO_WORKFLOWS_MAX_PARALLEL`, default 4); consecutive steps that share a `phase` label form a named synchronization barrier where ALL members fan out concurrently (same cap) and the next phase cannot start until every member settles — `parallel_group` and `phase` are mutually exclusive on the same step. Resume by passing `resume_state` OR by passing `resume_run_id` (the engine re-reads the on-disk journal at `<DATA_DIR>/workflow-runs/<runId>.json` and refuses if the workflow source SHA-256 has changed since pause). `shell` steps run ONE argv command each (no shell, no pipes; per-step timeout + 10 MB clamp; rendered command/stdin are re-validated against the shell-metachar guard so untrusted step output can't inject `$()` / backtick / pipe); `tool` steps dispatch `step.command` as a host tool name with JSON args from `stdin`, routed through `registry.execute()` — the same permission/approval/sandbox/plan-mode gates a normal tool call hits. The tool is `requiresConfirmation` (approve the whole run before any step executes) |
| `SUDO_WORKFLOWS_MAX_PARALLEL=N` | integer | 4 | Per-engine fan-out cap for members of a `parallel_group` OR a `phase` block. Outside a fan-out, steps still run sequentially. `1` is effectively-sequential and serves as a kill switch without touching workflow files. Invalid values fall back to 4. Re-read on each `meta.run-workflow` invocation, so changes take effect without process restart |
| `SUDO_WORKFLOWS_QUEUE=1` | boolean | off | Registers `meta.enqueue-workflow` and starts the cross-workflow scheduler. Initializes a `TaskQueue` + `TaskExecutor` over `<DATA_DIR>/mind.db` (the same SQLite that backs `meta.task-manager`), registers a `workflow.run` handler that loads + runs a workflow via the same engine `meta.run-workflow` uses, and starts the executor poll loop. Queued runs auto-approve internal `approval: true` step gates (no operator-present resume path); the enqueue tool REFUSES to enqueue a workflow with approval steps unless `auto_approve:true` is set. Each individual workflow run still uses `SUDO_WORKFLOWS_MAX_PARALLEL` for its own intra-workflow fan-out; the two caps multiply. Pending runs survive process restarts (TaskExecutor drains 'queued' tasks on start). Independent of `SUDO_WORKFLOWS` — operators can enable sync-only, queue-only, or both |
| `SUDO_WORKFLOWS_QUEUE_CONCURRENT=N` | positive integer | 2 | Maximum concurrent queued workflow runs in flight. Default 2 because workflows are heavyweight (each can spawn many child processes). Each run also consumes up to `SUDO_WORKFLOWS_MAX_PARALLEL` slots for its intra-workflow fan-out. Invalid values fall back to 2 |
| `SUDO_WORKFLOWS_QUEUE_POLL_MS=N` | integer >=100 | 5000 | TaskExecutor poll interval (ms) — how often the executor checks for newly-queued or unblocked tasks. Minimum 100. Invalid values fall back to 5000. Pending runs survive process restarts; the executor drains them on next start regardless of this interval |
| `SUDO_ACP_MODEL` | string | unset | Pin a specific model for the ACP agent subprocess (`pnpm acp` / `node dist/core/acp/acp-cli.js`). Default uses the Brain's smart-routing. Slice 2 (gap #26) added tool dispatch + ACP `session/request_permission` round-trip + new `session/update` variants (`tool_call`, `tool_call_update`, `thought`); the wire format for tool invocations is `<tool_call id="<stable-id>" name="<tool-name>">JSON-args</tool_call>` blocks in the streamed assistant text, parsed after each stream pass and dispatched via an injected `AcpToolHost`. Slice 3 wires a curated host over the standard ACP client methods: `fs.read_text_file` / `fs.write_text_file` (mapped to `fs/read_text_file` + `fs/write_text_file`) and `terminal.{create,output,wait_for_exit,kill,release}` (mapped to `terminal/*`). Writes / spawns / kills require permission; reads / waits / releases do not. `AcpClientFacade` types every client method; `makeJsonRpcClientFacade(conn)` is the default impl. The standalone ACP process does NOT load the full sudo-ai `ToolRegistry` — the curated fs/terminal set is enough for an editor-driven coding agent and matches the ACP spec exactly |
| `SUDO_ACP_TOOLS=0` | boolean | tools ON | Disable the fs/terminal tool host on the standalone ACP agent — reverts to slice-1 chat-only behavior. The host is on by default in slice 3 |
| `SUDO_ACP_PERSIST=0` | boolean | persist ON | Disable per-session JSON persistence (gap #26 slice 4). When persistence is on (default), the ACP agent writes histories under `<DATA_DIR>/acp-sessions/<sessionId>.json` and advertises `agentCapabilities.loadSession = true` at initialize. `session/load` reads the store and replays history as `user_message_chunk` + `agent_message_chunk` `session/update` notifications so the editor can rebuild its UI. SessionIds are confined to `[A-Za-z0-9_-]+` to block path traversal |

...
| `SUDO_WORKFLOWS=1` | boolean | off | Registers `meta.run-workflow`, the deterministic multi-step workflow runner. A workflow is a `.yaml` file of sequential steps under the workspace `workflows/` dir; the engine threads stdout via `stdin: '{{prev}}'`, gates steps with `condition` expressions, and pauses on `approval: true` steps returning a resume token (resume by passing `resume_state`, or set `auto_approve:true` to clear gates). `shell` steps run ONE argv command each (no shell, no pipes; per-step timeout + 10 MB clamp); `tool` steps dispatch `step.command` as a host tool name with JSON args from `stdin`, routed through `registry.execute()` — the same permission/approval/sandbox/plan-mode gates a normal tool call hits. The tool is `requiresConfirmation` (approve the whole run before any step executes). Sequential only; parallel fan-out and an on-disk resume journal are follow-up slices |

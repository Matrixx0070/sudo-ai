...
STEP 2: Take those character reference images and GIVE THEM to Grok when creating each SCENE IMAGE. Open Grok on the web browser, UPLOAD the character image, and say "using this character, create this scene: [scene description]". This keeps the characters looking the SAME in every scene. Do this for all 8 scenes.
...
This is the professional way — character refs first, then scenes with refs, then video from scenes.
...
for (let i = 0; i < 50; i++) {
  ...
  console.log(`Check ${i+1}/50 — response ready: ${hasResponse}`);
...
await page.screenshot({ path: '/root/sudo-ai-v3/screenshots/video-steps.png', fullPage: false });

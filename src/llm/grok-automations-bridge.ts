/**
 * @file grok-automations-bridge.ts
 * @description Node ↔ Python bridge for the Grok automations/tasks seat lane.
 * Spawns `scripts/grok-web/grok_automations.py` (curl_cffi — grok.com `/rest/*`
 * sits behind Cloudflare and 403s plain Node fetch). One JSON request on stdin,
 * one JSON response on stdout. Clone of the proven `grok-web-bridge.ts`
 * spawn/settle/timeout structure.
 *
 * SECRETS: the cookie header is a session secret. It is passed to the child on
 * stdin ONLY and is NEVER logged here (the python side never echoes it either).
 * Do not add debug logging of `req`.
 *
 * Same-host invariant: cf_clearance is IP-bound, so the python child must run
 * on the same machine as the browser that captured the session. This bridge
 * always spawns a LOCAL python3; it never makes a network hop of its own.
 */

import { spawn } from 'node:child_process';
import path from 'node:path';
import { PROJECT_ROOT } from '../core/shared/paths.js';
import { createLogger } from '../core/shared/logger.js';
import type { GrokWebCreds, GrokWebErrorClass } from './grok-web-bridge.js';

const log = createLogger('llm:grok-automations-bridge');

const SCRIPT_PATH = path.join(PROJECT_ROOT, 'scripts', 'grok-web', 'grok_automations.py');
const PYTHON_BIN = process.env['SUDO_GROK_WEB_PYTHON'] ?? 'python3';

/** Hard ceiling regardless of per-op timeouts (bridge-level guard). */
const HARD_TIMEOUT_MS = 60_000;

// ---------------------------------------------------------------------------
// Request / response types (verified live against the seat 2026-07-21)
// ---------------------------------------------------------------------------

export interface AutomationsListRequest {
  op: 'list';
  timeoutSec?: number;
}

export interface AutomationsCatalogRequest {
  op: 'catalog';
  timeoutSec?: number;
}

export interface AutomationsTasksRequest {
  op: 'tasks';
  timeoutSec?: number;
}

export interface AutomationsToolsRequest {
  op: 'tools';
  timeoutSec?: number;
}

/** Creates a grok-side automation that goes LIVE immediately (owner action). */
export interface AutomationsCreateRequest {
  op: 'create';
  name: string;
  prompt: string;
  /** YYYY-MM-DD — server rejects dates >1 year out for ONCE cadence. */
  dayOfYear: string;
  /** HH:MM (default 09:00). */
  timeOfDay?: string;
  /** IANA tz (default UTC). */
  timezone?: string;
  timeoutSec?: number;
}

export interface AutomationsDeleteRequest {
  op: 'delete';
  taskId: string;
  timeoutSec?: number;
}

export type GrokAutomationsRequest =
  | AutomationsListRequest
  | AutomationsCatalogRequest
  | AutomationsTasksRequest
  | AutomationsToolsRequest
  | AutomationsCreateRequest
  | AutomationsDeleteRequest;

/** Automation as returned live: {taskId, content{...}, isActive, schedules[], triggers[]}. */
export interface GrokAutomation {
  taskId: string;
  content?: {
    name?: string;
    prompt?: string;
    notificationMethod?: string;
    connectors?: unknown[];
    [k: string]: unknown;
  };
  isActive?: boolean;
  schedules?: Array<{
    scheduleId?: string;
    taskCadence?: string;
    isEnabled?: boolean;
    nextRun?: string | null;
    dayOfYear?: string;
    timeOfDay?: string;
    timezone?: string;
    isCompleted?: boolean;
    [k: string]: unknown;
  }>;
  triggers?: unknown[];
}

export interface GrokAutomationsResponse {
  ok: boolean;
  status?: number;
  errorClass?: GrokWebErrorClass;
  detail?: string;
  // op=list
  automations?: GrokAutomation[];
  // op=catalog — {groups:[{provider,displayName,triggers:[...],providerEnum}]}
  groups?: Array<{
    provider?: string;
    displayName?: string;
    triggers?: Array<{
      triggerType?: string;
      displayName?: string;
      description?: string;
      dimensions?: unknown[];
      [k: string]: unknown;
    }>;
    [k: string]: unknown;
  }>;
  // op=tasks — {tasks[], taskUsage{frequentUsage,frequentLimit,occasionalUsage,occasionalLimit}}
  tasks?: unknown[];
  taskUsage?: {
    frequentUsage?: number;
    frequentLimit?: number;
    occasionalUsage?: number;
    occasionalLimit?: number;
  };
  // op=tools — {tools:[{id,label,icon,toolIds,connectorIds}]}
  tools?: Array<{
    id?: string;
    label?: string;
    icon?: string;
    toolIds?: string[];
    connectorIds?: string[];
  }>;
  // op=create
  automation?: GrokAutomation;
  // op=delete
  deleted?: boolean;
}

/** Injectable spawn seam — real child_process by default, mocked in tests. */
export type SpawnFn = typeof spawn;

// ---------------------------------------------------------------------------
// Bridge call
// ---------------------------------------------------------------------------

/**
 * Run one automations operation. Resolves with the python response (including
 * structured `ok:false` errors); never rejects — transport failures come back
 * as `errorClass:"bridge_error"`.
 */
export function callGrokAutomationsBridge(
  req: GrokAutomationsRequest,
  creds: GrokWebCreds,
  spawnFn: SpawnFn = spawn,
): Promise<GrokAutomationsResponse> {
  const perOpMs =
    typeof req.timeoutSec === 'number' ? req.timeoutSec * 1000 + 15_000 : HARD_TIMEOUT_MS;
  const timeoutMs = Math.min(perOpMs, HARD_TIMEOUT_MS);

  return new Promise<GrokAutomationsResponse>((resolve) => {
    let settled = false;
    let stdout = '';
    let stderr = '';

    const child = spawnFn(PYTHON_BIN, [SCRIPT_PATH], { stdio: ['pipe', 'pipe', 'pipe'] });

    const settle = (r: GrokAutomationsResponse): void => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      try {
        child.kill('SIGKILL');
      } catch {
        /* already gone */
      }
      resolve(r);
    };

    const timer = setTimeout(() => {
      log.warn({ op: req.op, timeoutMs }, 'grok-automations bridge timed out');
      settle({ ok: false, errorClass: 'timeout', detail: `bridge timed out after ${timeoutMs}ms` });
    }, timeoutMs);

    child.stdout?.on('data', (d: Buffer) => {
      stdout += d.toString('utf8');
    });
    child.stderr?.on('data', (d: Buffer) => {
      stderr += d.toString('utf8');
    });
    child.on('error', (err: Error) => {
      settle({ ok: false, errorClass: 'bridge_error', detail: `spawn failed: ${err.message}` });
    });
    child.on('close', (code: number | null) => {
      const line = stdout.trim().split('\n').filter(Boolean).pop() ?? '';
      try {
        const parsed = JSON.parse(line) as GrokAutomationsResponse;
        // Never log secrets; log only the coarse outcome.
        log.debug(
          { op: req.op, ok: parsed.ok, status: parsed.status, errorClass: parsed.errorClass },
          'grok-automations bridge result',
        );
        settle(parsed);
      } catch {
        settle({
          ok: false,
          errorClass: 'bridge_error',
          detail: `no JSON from bridge (exit ${code}); stderr: ${stderr.slice(0, 200)}`,
        });
      }
    });

    // Secrets go in ONLY here, on stdin. Never logged.
    const payload = JSON.stringify({ ...req, ...creds });
    try {
      child.stdin?.write(payload);
      child.stdin?.end();
    } catch (err) {
      settle({ ok: false, errorClass: 'bridge_error', detail: `stdin write failed: ${String(err)}` });
    }
  });
}

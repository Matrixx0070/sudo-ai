...
import { sanitizeForPrompt } from '../brain/sanitize-for-prompt.js';
...
export class BootstrapRunner {
  ...
  private async _applyResults(): Promise<void> {
    ...
    const identityContent = [
      ...
      ``,
      `## Name`,
      sanitizeForPrompt(agentName ?? 'SUDO'),
      ``,
      `## Personality`,
      sanitizeForPrompt(vibe ?? 'balanced, helpful, and direct'),
      ``,
      `## Timezone`,
      sanitizeForPrompt(timezone ?? 'UTC'),
      ``,
      `## Core Goals`,
      sanitizeForPrompt(goals ?? 'Assist the owner with their priorities'),
      ``,
    ...
    const userContent = [
      ...
      ``,
      `## Name`,
      sanitizeForPrompt(ownerName ?? 'Owner'),
      ``,
      `## Preferences`,
      `- Timezone: ${sanitizeForPrompt(timezone ?? 'UTC')}`,
      ``,
      `## Goals`,
      sanitizeForPrompt(goals ?? 'Not specified'),
      ``,

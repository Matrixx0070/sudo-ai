...
const CATEGORY_MAP: Record<CategoryName, CategoryRule> = {
  ...
  media: {
    ...
    patterns: [/\.(png|jpg|jpeg|gif|mp4|webm|svg)\b/i, /\bqr\s*code\b/i, /\b(org\s*chart|mind\s*map|tree\s*diagram|flow\s*chart)\b/i, /\bcode\s*(image|screenshot|snippet)\b/i, /\bsyntax\s*highlight/i, /\b(equation|formula|latex)\b/i, /\b(animated|animation|gif)\b/i],
    priority: 7,
    maxFromCategory: 4,
  },
  research: {

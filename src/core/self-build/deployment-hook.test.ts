...
function createMockDeps() {
  return {
    githubIssues: { addComment: vi.fn().mockResolvedValue({ success: true }) },
    metrics: { recordEvent: vi.fn() },
  };
}
...
describe('DeploymentHook', () => {
  ...
  beforeEach(() => {
    ...
    mockExecFile.mockImplementation((_cmd, _args, _opts, cb) => {
      ...
      if (!response) { callback(new Error(`Unexpected call #${callIdx}`), '', ''); return; }
      if (response.type === 'success') { callback(null, response.stdout, response.stderr); }
      else if (response.type === 'exit') {
        const err = new Error(response.stderr || 'Failed') as Error & { status: number };
        err.status = response.code; callback(err, response.stdout, response.stderr);
  ...
  afterEach(() => { delete process.env['GITHUB_TOKEN']; hook.cleanup(); });
  ...
  describe('monitorPR', () => {
    ...
    it('does not start duplicate monitor', () => {
      hook.monitorPR(42, 100);
      hook.monitorPR(42, 100);
    });
  });
  ...
  describe('stopMonitoring', () => {
    it('clears interval and removes from timers', () => {
      hook.monitorPR(42, 100);
  ...
  describe('checkAndDeploy', () => {
    ...
    it('does nothing when PR is open', async () => {
      mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ state: 'open', merged: false, head: { sha: 'abc' } }) });
      await hook.checkAndDeploy(42, 100);
      expect(mockFetch).toHaveBeenCalled();
      expect(mockExecFile).not.toHaveBeenCalled();
    });
    it('runs CI and deploys when PR is merged', async () => {
      mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ state: 'merged', merged: true, head: { sha: 'abc' } }) });
      queueExec({ type: 'success', stdout: 'lint ok', stderr: '' }, { type: 'success', stdout: 'tests', stderr: '' }, { type: 'success', stdout: 'pm2 ok', stderr: '' });
      await hook.checkAndDeploy(42, 100);
      expect(mockExecFile).toHaveBeenCalledTimes(3);
      expect(deps.githubIssues.addComment).toHaveBeenCalledWith(100, expect.stringContaining('SUCCESS'));
      expect(deps.metrics.recordEvent).toHaveBeenCalledWith('deployed', expect.any(Object));
    });
    it('rolls back when CI fails', async () => {
      mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ state: 'merged', merged: true, head: { sha: 'abc' } }) });
      queueExec({ type: 'success', stdout: 'lint ok', stderr: '' }, { type: 'exit', stdout: '', stderr: 'error', code: 1 }, { type: 'success', stdout: 'reset', stderr: '' });
      await hook.checkAndDeploy(42, 100);
      expect(mockExecFile).toHaveBeenCalledTimes(3);
      expect(deps.githubIssues.addComment).toHaveBeenCalledWith(100, expect.stringContaining('FAILED'));
    ...
    it('handles GitHub API error', async () => {
      ...
      await hook.checkAndDeploy(42, 100);
      expect(deps.metrics.recordEvent).toHaveBeenCalledWith('deploy_error', expect.objectContaining({ error: expect.any(String) }));
    });
  });

  describe('runCI', () => {
    it('returns passed when lint and test succeed', async () => {
      queueExec({ type: 'success', stdout: 'lint ok', stderr: '' }, { type: 'success', stdout: 'tests', stderr: '' });
    ...
    it('returns failed when test fails', async () => {
      queueExec({ type: 'success', stdout: 'lint ok', stderr: '' }, { type: 'exit', stdout: '', stderr: 'test error', code: 1 });
      const result = await hook.runCI();
      expect(result.passed).toBe(false);
  ...
  describe('rollback', () => {
    it('resets to previous commit', async () => {
      ...
      expect(mockExecFile).toHaveBeenCalledWith('git', ['reset', '--hard', 'abc123'], expect.objectContaining({ cwd: '/root/sudo-ai-v4' }), expect.any(Function));
  ...
  describe('addDeploymentComment', () => {
    it('adds success comment', async () => {
      await hook.addDeploymentComment(100, { success: true, action: 'deployed', output: 'OK' });
      expect(deps.githubIssues.addComment).toHaveBeenCalledWith(100, expect.stringContaining('SUCCESS'));
    });
    it('adds failure comment', async () => {
      await hook.addDeploymentComment(100, { success: false, action: 'rolled-back', output: 'CI failed' });
      expect(deps.githubIssues.addComment).toHaveBeenCalledWith(100, expect.stringContaining('FAILED'));
    });
    it('handles addComment error gracefully', async () => {
      ...
      await expect(hook.addDeploymentComment(100, { success: true, action: 'deployed' })).resolves.toBeUndefined();
    });
  });

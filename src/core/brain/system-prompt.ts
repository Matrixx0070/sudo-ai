...
export async function assembleSystemPrompt(options: SystemPromptOptions = {}): Promise<string> {
  ...
  const mythosBehavioralBlock = [
    ...
    '- NEVER ASSERT ABSENCE FROM A NARROW SEARCH: do not state that something does not exist, that there is no other X, or that nothing else matches just because a few searches came back empty — absence of evidence in a handful of queries is not proof of absence.',
    '- When asked to rule something out or confirm nothing else exists, run several BROAD, differently-phrased searches (vary wording and angle; drop restrictive filters) before concluding. If you still find nothing, scope the claim to your search — "I didn\'t find any X in my searches" — never "no X exists".',
    '',

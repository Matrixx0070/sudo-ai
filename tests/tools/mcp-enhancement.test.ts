 ...
 * @description Tests for MCP enhancements: OAuth, SSE transport, WebSocket transport, tool filtering, REST routes.
 */
...
} from '../../src/core/tools/mcp-routes.js';
...
describe('mcp-routes', () => {
  ...
  describe('handleListServers', () => {
    it('should return empty list when no servers', async () => {
      ...
      expect(result.body).toEqual({ servers: [] });
    });

    it('should return 503 when MCP disabled', async () => {
      process.env['SUDO_MCP_DISABLE'] = '1';
      const result = await handleListServers();
  ...
  describe('handleRegisterServer', () => {
    it('should reject missing body', async () => {
      const result = await handleRegisterServer(null);
      expect(result.status).toBe(400);
      expect((result.body as { error: string }).error).toBe('Request body required');
    });

    it('should reject missing name', async () => {
      const result = await handleRegisterServer({ url: 'http://test' });
      expect(result.status).toBe(400);
      expect((result.body as { error: string }).error).toBe('name is required (string)');
    });

    it('should reject missing url', async () => {
      const result = await handleRegisterServer({ name: 'test' });
      expect(result.status).toBe(400);
      expect((result.body as { error: string }).error).toBe('url is required (string)');
    });
    ...
    it('should reject invalid trustTier', async () => {
      const result = await handleRegisterServer({
        name: 'test',
        url: 'http://test',
        trustTier: 'invalid',
    ...
    it('should register valid server', async () => {
      const result = await handleRegisterServer({
        name: 'Test Server',
        url: 'http://test.com',
      ...
      const body = result.body as { server: { id: string; name: string } };
  ...
  describe('handleDeleteServer', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleDeleteServer('unknown');
      expect(result.status).toBe(404);
    });

    it('should delete existing server', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleDeleteServer(server.id);
      expect(result.status).toBe(200);
      expect((result.body as { success: boolean }).success).toBe(true);
      expect(getMcpServer(server.id)).toBe(undefined);
    });
  });

  describe('handleConnectServer', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleConnectServer('unknown', {});
      expect(result.status).toBe(404);
    });

    it('should fail to connect to invalid endpoint', async () => {
      const server = registerMcpServer('Test', 'http://invalid:9999');
      const result = await handleConnectServer(server.id, {});
      // Will fail to connect, but should handle gracefully
      expect(result.status).toBeGreaterThanOrEqual(400);
    });
  });

  describe('handleDisconnectServer', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleDisconnectServer('unknown');
      expect(result.status).toBe(404);
    });

    it('should disconnect existing server', async () => {
      const server = registerMcpServer('Test', 'http://test');
      connectMcpServer(server.id);
      const result = await handleDisconnectServer(server.id);
      expect(result.status).toBe(200);
      expect((result.body as { success: boolean }).success).toBe(true);
    });
  });
  ...
  describe('handleListTools', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleListTools('unknown');
      expect(result.status).toBe(404);
    });

    it('should return empty tools for server without tools', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleListTools(server.id);
      expect(result.status).toBe(200);
  ...
  describe('handleToggleTool', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleToggleTool('unknown', 'tool1', { enabled: true });
      expect(result.status).toBe(404);
    });

    it('should reject missing body', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleToggleTool(server.id, 'tool1', null);
      expect(result.status).toBe(400);
    });

    it('should reject missing enabled field', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleToggleTool(server.id, 'tool1', {});
      expect(result.status).toBe(400);
    });

    it('should return 404 for unknown tool', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleToggleTool(server.id, 'unknown', { enabled: true });
      expect(result.status).toBe(404);
    });
  });

  describe('handleGetServerStatus', () => {
    it('should return 404 for unknown server', async () => {
      const result = await handleGetServerStatus('unknown');
      expect(result.status).toBe(404);
    });

    it('should return server status', async () => {
      const server = registerMcpServer('Test', 'http://test');
      const result = await handleGetServerStatus(server.id);
      expect(result.status).toBe(200);
      const body = result.body as { server: { id: string } };
      expect(body.server.id).toBe(server.id);
    });
  });
});
